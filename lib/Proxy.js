...
function Proxy_get(url,        // @arg URLString:
                   callback) { // @arg Function: callback(error, responseText, xhr):void
                               // @ret this:
//{@assert
    _if(!Valid.type(url,      "String") || URI.parse(url).error, Proxy.prototype.get, "url");
    _if(!Valid.type(callback, "Function"),                       Proxy.prototype.get, "callback");
//}@assert
...
//{@assert
    _if(!Valid.type(name, "String"), Proxy.prototype.getResponseHeader, "name");
//}@assert
...
function Proxy_open(method,     // @arg String: "GET" or "POST", ...
                    ...
                    password) { // @arg String(= ""):
                                // @help: Proxy#open
//{@assert
    _if(this._xhr["readyState"] !== READY_STATE_UNSENT,     Proxy.prototype.open, "sequence error");
    _if(!Valid.type(method, "String")  || !/^(GET|POST)$/.test(method),
                                                            Proxy.prototype.open, "method");
    _if(!Valid.type(url,    "String")  || URI.parse(url).error,
                                                            Proxy.prototype.open, "url");
    _if(!Valid.type(async,  "Boolean/omit"),                Proxy.prototype.open, "async");
    _if(!Valid.type(user,   "String/omit"),                 Proxy.prototype.open, "user");
    _if(!Valid.type(password, "String/omit"),               Proxy.prototype.open, "password");
//}@assert
...
//{@assert
    _if(!Valid.type(mimeType, "String"), Proxy.prototype.overrideMimeType, "mimeType");
//}@assert
...
function Proxy_send(data) { // @arg Any(= null): POST request body
                            // @help: Proxy#send
//{@assert
    _if(this._xhr["readyState"] !== READY_STATE_OPENED, Proxy.prototype.send, "sequence error");
//}@assert
...
//{@assert
    _if(!Valid.type(name,  "String"), Proxy.prototype.setRequestHeader, "name");
    _if(!Valid.type(value, "String"), Proxy.prototype.setRequestHeader, "value");
//}@assert
...
//}@node
if (global["Proxy"]) {
    global["Proxy_"] = Proxy; // secondary
} else {
    global["Proxy"]  = Proxy; // primary
}
